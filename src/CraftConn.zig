...
pub fn writePacketWithEncoding(
    ...
    const payload_length = try craft_io.length(packet, allocator, try diag.child(.packet_body), encoding);
    const id_length = try craft_io.length(id, allocator, try diag.child(.packet_id), .varnum);
    ...
    if (conn.compression) |*comp| {
        ...
        if (data_length > comp.threshold) {
            ...
            const data_length_length = try craft_io.length(data_length, allocator, try diag.child(.packet_data_length), .varnum);
            ...
            _ = try craft_io.encode(id, compressor_writer, allocator, try diag.child(.packet_id), .varnum);
            _ = try craft_io.encode(packet, compressor_writer, allocator, try diag.child(.packet_body), encoding);
            ...
            const total_length_length = try craft_io.length(total_length, allocator, try diag.child(.packet_length), .varnum);
            ...
            // encode total length
            _ = try craft_io.encode(total_length, prefix_writer, allocator, try diag.child(.packet_length), .varnum);
            // encode data length
            _ = try craft_io.encode(@as(i32, @intCast(data_length)), prefix_writer, allocator, try diag.child(.packet_data_length), .varnum);
            ...
            // encode total_length
            _ = try craft_io.encode(@as(i32, @intCast(data_length + 1)), buf_writer, allocator, try diag.child(.packet_length), .varnum);
            // encode data_length=0
            try buf_writer.writeByte(0);
            // encode id + payload
            _ = try craft_io.encode(id, buf_writer, allocator, try diag.child(.packet_id), .varnum);
            _ = try craft_io.encode(packet, buf_writer, allocator, try diag.child(.packet_body), encoding);
            bytes_to_write = conn.buf.items;
        }
    } else {
        // compression is disabled, so we just write total_length, id, packet
        _ = try craft_io.encode(@as(i32, @intCast(data_length)), buf_writer, allocator, try diag.child(.packet_length), .varnum);
        _ = try craft_io.encode(id, buf_writer, allocator, try diag.child(.packet_id), .varnum);
        _ = try craft_io.encode(packet, buf_writer, allocator, try diag.child(.packet_body), encoding);
        bytes_to_write = conn.buf.items;
    }
